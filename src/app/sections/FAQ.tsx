      ...
      <div>
        {faqs.map((faq, index) => (
          <div key={index} className="mb-4 border border-gray-300 rounded-lg overflow-hidden">
            <button
              className={`flex justify-between w-full text-left px-5 py-4 text-sm md:text-lg font-medium transition-all duration-300 ease-in-out ${openIndex === index ? 'bg-[#520052] text-white' : 'bg-white text-gray-900'}`}
